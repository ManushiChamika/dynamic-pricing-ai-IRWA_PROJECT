...
const useToasts = create<{
  toasts: { id: number; type: 'info' | 'success' | 'error'; text: string }[]
  push: (t: { text: string; type?: 'info' | 'success' | 'error' }) => void
  remove: (id: number) => void
}>(set => ({
  toasts: [],
  push: ({ text, type = 'info' }) => set(s => ({
...
function Toasts() {
  ...
  return (
    <div className="fixed right-5 bottom-5 flex flex-col gap-3 z-50" aria-live="polite" role="status">
      {toasts.map(t => (
        <div key={t.id} className={`backdrop-blur-2xl bg-panel-solid border rounded-xl px-4 py-3 shadow-lg max-w-[380px] animate-slideInRight text-sm font-medium ${t.type === 'info' ? 'border-blue-500 bg-gradient-to-br from-blue-500/15 to-panel-solid' : t.type === 'success' ? 'border-green-500 bg-gradient-to-br from-green-500/15 to-panel-solid' : 'border-red-500 bg-gradient-to-br from-red-500/15 to-panel-solid'}`} onClick={() => remove(t.id)} title="Click to dismiss">
          {t.text}
        </div>
      ))}
    </div>
...
// --- Simple API helper (reuses backend auth query param semantics)
const api = async (url: string, init?: RequestInit & { json?: any }) => {
...
const useSettings = create<Settings & { set: (p: Partial<Settings>) => void; settingsOpen: boolean; setSettingsOpen: (open: boolean) => void }>((set) => ({
...
const useThreads = create<ThreadsState>((set, get) => ({
  threads: [],
  currentId: null,
...
type MessagesState = {
 ...
 const useMessages = create<MessagesState>((set, get) => ({
  ...
  send: async (threadId, content, user_name, stream) => {
    if (!stream) {
      await api(`/api/threads/${threadId}/messages`, { method: 'POST', json: { content, user_name } })
      await get().refresh(threadId)
      return
    }
    // SSE streaming with AbortController and stop support
    await get().refresh(threadId)
    const live: Message = { id: -1, role: 'assistant', content: '' }
    set(s => ({ messages: [...s.messages, live], streamingActive: true, liveActiveAgent: null, liveAgents: [], liveTool: null, turnStats: null }))
    const token = localStorage.getItem('token') || ''
    ...
    let finished = false
    try {
      const resp = await fetch(url, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ content, user_name }), signal: ctrl.signal })
      if (!resp.ok) {
        ...
        while (true) {
          ...
          while ((idx = buf.indexOf('\n\n')) !== -1) {
            ...
            if (ev === 'thinking') {
              const s = (useSettings as any).getState?.() as Settings | undefined
              if (s?.showThinking && !thinkingShown) {
                thinkingShown = true
                set(st => ({ messages: [...st.messages, { id: -2, role: 'assistant', content: 'Thinking…' }] }))
              }
              continue
            }
            if (ev === 'agent' && data) {
                ...
                if (obj.delta) {
                  set(s => {
                    ...
                    if (thinkingShown) {
                      ...
                      if (last && last.id === -2) { arr.pop(); thinkingShown = false }
                    }
                    const last = arr[arr.length - 1]
                  ...
                  // Capture per-turn stats for header display
      ...
      // Ignore abort errors; treat others similarly
    } finally {
      set({ controller: null, streamingActive: false, liveActiveAgent: null, liveTool: null })
      await get().refresh(threadId)
    }
  ...
  branch: async (threadId, parentId, content, user_name) => {
    await api(`/api/threads/${threadId}/messages`, { method: 'POST', json: { content, user_name, parent_id: parentId } })
  },
...
function Sidebar() {
  ...
  useEffect(() => { document.querySelector('.sidebar')?.classList.toggle('collapsed', collapsed) }, [collapsed])
      ...
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
          <Button variant="ghost" size="icon" aria-label={collapsed ? 'Expand sidebar' : 'Collapse sidebar'}
            onClick={() => setCollapsed(c => { const n = !c; localStorage.setItem('sidebarCollapsed', n ? '1' : '0'); return n })}
            aria-expanded={!collapsed}
            aria-controls="thread-list"
          >{collapsed ? '⮞' : '⮜'}</Button>
          <Button onClick={() => createThread()} aria-label="Create new thread" style={{ flex: 1 }}>+ New Chat</Button>
        </div>
...
function MessageView({ m, showModel, showTimestamps, showMeta, lastUserId, allMessages }: { m: Message, showModel: boolean, showTimestamps: boolean, showMeta: boolean, lastUserId?: number, allMessages: Message[] }) {
  const { del, refresh, streamingActive, messages } = useMessages()
  ...
  return (
    ...
    >
      ...
      {m.id === -1 ? (
        <div className="mt-2 opacity-70 text-xs flex gap-2 flex-wrap mb-1" style={{ marginBottom: 4 }}>
          {liveActiveAgent ? <span className="text-xs opacity-95 border border-accent px-3 py-1 rounded-xl bg-gradient-to-br from-accent-light to-purple-500/15 text-accent font-medium transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] shadow-[0_2px_4px_rgba(99,102,241,0.1)]">Agent: {liveActiveAgent}</span> : null}
          {liveTool ? (
            <span className={`text-xs opacity-95 border border-accent px-3 py-1 rounded-xl bg-gradient-to-br from-accent-light to-purple-500/15 text-accent font-medium transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] shadow-[0_2px_4px_rgba(99,102,241,0.1)] ${liveTool.status === 'done' ? 'opacity-70' : ''}`}>
              {liveTool.status === 'running' ? <span className="inline-block w-3.5 h-3.5 border-2 border-accent-light border-t-accent rounded-full animate-spin mr-1.5"/> : null}
      ...
      <div className="flex gap-1.5 mt-2 opacity-0 transition-opacity duration-200 hover:opacity-100">
        <Button variant="ghost" size="sm" onClick={() => navigator.clipboard.writeText(m.content || '')} disabled={streamingActive} aria-label="Copy message">Copy</Button>
        ...
        }} disabled={streamingActive} aria-label="Delete message">Del</Button>
        <BranchButtons m={m} />
      </div>
      <div className="mt-2 opacity-70 text-xs flex gap-2 flex-wrap">
        {showTimestamps && m.created_at ? <span className="time" title={m.created_at}>{new Date(m.created_at).toLocaleString()}</span> : null}
        {showMeta ? (
          <MetadataTooltip message={m} />
        ) : null}
        {m.role === 'assistant' && showModel && (m.model || m.metadata?.provider) ? (
          <span className="opacity-60 text-[11px] ml-auto" title={m.model || ''}>{m.metadata?.provider ? `${m.metadata.provider}${m.model ? ':' + m.model : ''}` : m.model}</span>
        ) : null}
      </div>
    </animated.div>
  )
...
function buildInfo(m: Message) {
  const info: string[] = []
  if (m.created_at) info.push(`time: ${new Date(m.created_at).toLocaleString()}`)
  const modelComputed = m.model || (m.metadata && (m.metadata as any).provider ? `${(m.metadata as any).provider}${m.model ? `:${m.model}` : ''}` : '')
  ...
  const agentNames = m.agents?.activated || []
  const agentCount = m.agents?.count ?? agentNames.length
  if (agentCount > 0) info.push(`agents: ${agentCount}${agentNames.length ? ` (${agentNames.join(', ')})` : ''}`)
  const toolNames = m.tools?.used || []
  const toolCount = m.tools?.count ?? toolNames.length
  if (toolCount > 0) info.push(`tools: ${toolCount}${toolNames.length ? ` (${toolNames.join(', ')})` : ''}`)
  return info.join('\n')
}
...
function EditButton({ m }: { m: Message }) {
  ...
  return (
    ...
    }} disabled={streamingActive} aria-label="Edit message">Edit</Button>
  )
}
...
function BranchButtons({ m }: { m: Message }) {
  ...
  return (
    <>
      ...
      }} disabled={streamingActive} aria-label="Branch conversation here">Branch</Button>
      <Button variant="ghost" size="sm" onClick={async () => {
        if (!currentId) return
        usePrompt.getState().openPrompt({
      ...
      }} disabled={streamingActive} aria-label="Fork new thread from here">Fork</Button>
    </>
  )
}
            ...
            <input type="file" style={{ display:'none' }} aria-label="Import thread file" onChange={async (e) => {
              ...
              try {
                ...
                if (res.ok && res.data?.id) {
                  ...
                  useToasts.getState().push({ type: 'success', text: `Imported to thread #${res.data.id}` })
                } else {
  ...
  return (
    <aside className={`${collapsed ? 'w-12' : 'w-[280px]'} border-l border-border p-4 overflow-auto bg-[rgba(17,24,39,0.85)] backdrop-blur-3xl transition-all duration-300`} aria-label="Prices panel">
      <div className="flex gap-2 items-center mb-2">
        ...
        {!collapsed ? (
          <>
            <Input placeholder="sku (optional)" value={sku} onChange={e => setSku(e.target.value)} className="flex-1" />
            <Button variant="ghost" size="icon" onClick={() => setViewMode(v => v === 'sparkline' ? 'chart' : 'sparkline')} 
              aria-label={viewMode === 'sparkline' ? 'Switch to chart view' : 'Switch to sparkline view'}
              title={viewMode === 'sparkline' ? 'Chart view' : 'Sparkline view'}>
              {viewMode === 'sparkline' ? '📊' : '📈'}
            </Button>
            <Button variant="ghost" size="icon" onClick={() => setRunning(r => !r)} aria-label={running ? 'Pause stream' : 'Resume stream'}>
              {running ? '⏸' : '▶'}
            </Button>
          </>
        ) : null}
      ...
      {!collapsed ? (
        <div ref={pricesParent as any} className="grid gap-2">
          {keys.length === 0 ? <div className="text-center py-12 px-6 text-muted text-base">Waiting for prices…</div> : null}
          {keys.map(k => {
            const data = prices[k] || []
            ...
            return (
              <div key={k} className="border border-border rounded-2xl p-4 bg-panel backdrop-blur-xl shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md">
                <div className={`flex justify-between items-center ${viewMode === 'chart' ? 'mb-3' : 'mb-2'}`}>
                ...
                {viewMode === 'sparkline' ? (
                  ...
                  <Suspense fallback={<div className="text-center py-4 text-muted text-sm">Loading chart…</div>}>
