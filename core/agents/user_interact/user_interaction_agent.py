...
class UserInteractionAgent:
    def __init__(self, user_name, mode: str = "user", owner_id: Optional[str] = None):
        ...
        s = get_settings()
        self.api_key = getattr(s, "openrouter_api_key", None) or os.getenv("OPENROUTER_API_KEY")
        self.base_url = getattr(s, "openrouter_base_url", None) or os.getenv("OPENROUTER_BASE_URL", "https://openrouter.ai/api/v1")
        self.model_name = getattr(s, "openrouter_model", None) or os.getenv("OPENROUTER_MODEL", "deepseek/deepseek-r1-0528:free")
        self.keywords = [
        ...
        try:
            if get_llm_client is not None:
                ...
                if llm.is_available():
                    ...
                    try:
                        s = get_settings()
                        ui_max_tokens = getattr(s, "ui_llm_max_tokens", None)
                        if ui_max_tokens is None:
                            ui_max_tokens = int(os.getenv("UI_LLM_MAX_TOKENS", "0") or "0")
                    except Exception:
                        ui_max_tokens = 0
                    max_tokens_cfg = ui_max_tokens if ui_max_tokens and ui_max_tokens > 0 else 1024
